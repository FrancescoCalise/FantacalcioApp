...
export class AddCentrocampistaInTeamModalComponent {
  ...
  constructor(public activeModal: NgbActiveModal, private service: ApiService, private toastr: ToastrService) {
    this.service.allPlayers().subscribe(
      (data: Player[]) => {
        this.players = data;
        this.playerFilter = data.filter(p => p.role === 'C' && p.teamFantaId === '' && !p.name.endsWith('*'));
        ...
        this.soltEmpty = 8 - this.myPlayers.filter(t => t.role === 'C').length;
      },
      (err) => {
      }
    );
