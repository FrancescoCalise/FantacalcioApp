...
export class AddDifensoreInTeamModalComponent {
  ...
  constructor(public activeModal: NgbActiveModal, private service: ApiService, private toastr: ToastrService) {
    this.service.allPlayers().subscribe(
      (data: Player[]) => {
        this.players = data;
        this.playerFilter = data.filter(p => p.role === 'D' && p.teamFantaId === '' && !p.name.endsWith('*'));
        ...
        this.soltEmpty = 8 - this.myPlayers.filter(t => t.role === 'D').length;
      },
      (err) => {
      }
    );
